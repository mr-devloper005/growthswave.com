  ...
  return (
    <EditableSiteShell>
      <main className="mx-auto max-w-[1600px] px-4 py-10 sm:px-6 lg:px-8">
        <section className="rounded-[2.4rem] border border-black/[0.08] bg-white p-6 shadow-[0_24px_80px_rgba(75,46,43,0.10)] sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <button type="button" className="rounded-full border border-black/[0.08] bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(255,248,240,0.96))] px-4 py-2 text-sm font-black transition hover:-translate-y-0.5" onClick={refreshComments}>
              Refresh comments
            </button>
          </div>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-md">
              ...
              <input
                ...
                className="h-11 w-full rounded-2xl border border-black/[0.08] bg-white pl-9 pr-3 text-sm outline-none"
              />
            </div>
        ...
        {visibleComments.length ? (
          <section className="mt-8 grid gap-4">
            {visibleComments.map((item) => (
              <article key={`${item.articleSlug}-${item.id}`} className="rounded-[2rem] border border-black/[0.08] bg-white p-5 shadow-[0_18px_44px_rgba(75,46,43,0.08)]">
          ...
          <section className="mt-8 rounded-[2rem] border border-dashed border-black/[0.08] bg-white/70 p-8 text-center">
        ...
        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-[2rem] border border-black/[0.08] bg-white p-4 text-sm text-[var(--slot4-muted-text)]">
            <span>
              Page {currentPage} of {totalPages}
            </span>
            <div className="flex gap-2">
              <button type="button" className="rounded-full border border-black/[0.08] px-4 py-2 font-black disabled:opacity-40" disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>
                Previous
              </button>
              <button type="button" className="rounded-full border border-black/[0.08] px-4 py-2 font-black disabled:opacity-40" disabled={currentPage >= totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>
                Next
              </button>
            </div>
          </div>
